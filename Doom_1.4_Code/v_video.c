...
void V_Init (void)
{
    ...
    printf ("Resolution: %d x %d, AR: %d\n", SCREENWIDTH, SCREENHEIGHT, weirdaspect);
	...
	vid_mem = malloc_64(vid_size);
    if (vid_mem == NULL)
      I_Error ("Couldn't allocate memory for screens\n");
	vid_mem = (byte *)((int)vid_mem | 0x40000000); // uncached access
	sceKernelDcacheWritebackInvalidateAll();

    printf ("Screen memory located at 0x%08x\n", (unsigned int)vid_mem);

    for (i=0 ; i<4 ; i++)
		screens[i] = vid_mem + i*SCREENWIDTH*SCREENHEIGHT;
